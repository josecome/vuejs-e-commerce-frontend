...
const link = ref(`${root_link.value}/api/products_for_sale_all}`)
...
const Product_in_cart = async (id, v) => {
  ...
  if (v === 'p') {
    ...
    await axios.delete(`/delete_item_in_cart/${id}`).then(
      (response) => {
        rs_response = response.data
      },
      (error) => {
        ...
        <div v-for="product_item in list_of_products" :key="id" class="col">
          <div class="card shadow-sm">
            <text x="80%" y="80%" fill="#eceeef" dy=".3em">
              <img
                :src="`${root_link}/storage/images/products/` + product_item.image_link"
                @click="
                  setSelectedProduct(
                    product_item.product,
                    product_item.description,
                    product_item.image_link
                  ),
                    showProdModal('p')
                "
            ...
            <div class="card-body" style="background-color: #d4e6f1">
              ...
              <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group">
                  ...
                  >
                    {{
                      product_status[product_item.id] !== null &&
                      product_status[product_item.id] === 1
                        ? 'Remove from Cart'
                        : 'Add to Cart'
                    }}
                  </button>
                ...
                <div
                  v-if="product_status[product_item.id] === 1"
      ...
      <div id="prodDetail" class="modal-body">
        <table>
          <tr>
            <td>
              <img
                :src="`${root_link}/storage/images/products/${selected_image_link}`"
          ...
          <tr>
            <td colspan="2">
              ...
              <button @click="hideProdModal('p')" class="btn btn-secondary rounded-pill">
                Continue Shopping
              </button>
              <button
                @click="MarkAsOrdered(), showProdModal('c')"
                style="padding: 10px"
                class="btn btn-secondary rounded-pill"
              >
                Checkout
              ...
              <div v-for="product_related in list_of_products.slice(1, 4)">
                <table>
                  <tr>
                    <td>
                      <img
                        :src="`${root_link}/storage/images/products/${product_related.image_link}`"
                  ...
                  <tr>
                    <td>
                      <button
                        @click="checkProduct($event, product_related.id, product_related.product)"
                      >
                        Add to Cart
                      </button>
                    </td>
    ...
    <template #body>
      <div id="prodInCart" class="modal-body">
        <table>
          ...
          <tbody>
            <tr
              v-for="(product_item_in_cart, index) in Products_in_Cart"
              :key="id"
              class="col"
            ...
            >
              ...
              <td>
                <i
                  @click="Product_in_cart(product_item_in_cart.id, 'r')"
                  class="bi bi-x-octagon-fill"
                  style="float: right; padding-left: 6px; color: #e74c3c"
                ></i>
              </td>
            </tr>
            <tr>
